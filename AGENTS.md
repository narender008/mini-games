...
- Block Tower debug switches: `?debug` exposes `window.__bt` (freeze, thaw, step(ms), drop, tower(n), knock, sweep, choose(key, id), play, menu, place(shape, x, y, turns), fill, state), `?play` starts straight into a game, `?mode=little|big`, `?set=`, `?room=`, `?tool=`, `?challenge=`, `?cover` hides UI, plus `?quality=`, `?msaa=`, `?shadows=0`, `?ao=0`, `?dof=0`, `?tone=`. Physics is Rapier (`vendor/rapier/`); settled blocks are frozen as sleeping kinematic bodies (not fixed ones, which trap Rapier 0.20 on removal) so towers stand perfectly still, see the header of `games/block-tower/src/physics.js`.
- Marble Run debug switches: `?debug` exposes `window.__mr` (freeze, thaw, step(ms), drop, level(id), mode(id), follow, view(az, el, dist, tx, ty, tz) in degrees and metres, home, marbles, layout, stats, and for Big kid build(action, arg) and solve), `?play` starts straight into a game, `?level=sunny|crystal|storybook|cosmic`, `?mode=little|big`, `?cover` hides UI, plus `?quality=`, `?msaa=`, `?shadows=0`, `?ao=0`, `?dof=0`, `?tone=`. Marbles run on the game's own track physics, not Rapier: pieces are lanes on a 5 cm grid joined port to port (`src/track/`), stepped at 240 Hz in `src/sim.js`.
