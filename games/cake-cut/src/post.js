// Post-processing: an HDR render with depth, then a lens pass that adds
// ambient occlusion (from the depth buffer, at half resolution) and a
// shallow depth of field so the room behind the table melts into bokeh,
// then bloom for the candle flames and fairy lights, filmic (ACES) tone
// mapping, and a warm grade with a light vignette and grain.
...
void main() {
  ...
  for (int i = 0; i < SAMPLES; i++) {
    ...
    occ += max(0.0, vn - 0.0015 * -P.z) / (vv + 0.00005) * fall * fall;
  }
...
class LensPass extends Pass {
  constructor(camera, quality) {
    ...
    this.aoMat = new THREE.ShaderMaterial({
      ...
      uniforms: { ...common(), uRes: { value: new THREE.Vector2() }, uProj: { value: new THREE.Matrix4() }, uRadius: { value: 0.028 }, uIntensity: { value: 0.9 } },
      depthTest: false,
      depthWrite: false,
    });
  ...
  setSize(w, h) {
    ...
    // blur size and bokeh scale with the picture height
    this.lensMat.uniforms.uAperture.value = 7 * (h / 1000);
    this.lensMat.uniforms.uMaxCoC.value = 16 * (h / 1000);
  }
...
void main() {
  ...
  // a touch of warmth in the shadows, and saturation back after ACES
  float l = dot(c.rgb, vec3(0.2126, 0.7152, 0.0722));
  c.rgb = mix(vec3(l), c.rgb, 1.12);
  c.rgb += vec3(0.012, 0.004, -0.006) * (1.0 - l);
  vec2 d = vUv - 0.5;
...
export class Post {
  constructor(renderer, scene, camera, quality) {
    ...
    this.bloom = new UnrealBloomPass(new THREE.Vector2(size.x, size.y), 0.3, 0.55, 1.15);
