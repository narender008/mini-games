...
void shadeInclusions(vec3 p, vec3 cream, vec3 fruit, float px, float slices, out vec3 alb, out float rough, out float h) {
  ...
  vec3 q = slices > 0.5 ? vec3(p.x * 150.0, p.y * 430.0, p.z * 150.0) : vec3(p.x, p.y * 1.3, p.z) * 190.0;
  vec3 w = worley(q + 3.3);
  float keep = step(slices > 0.5 ? 0.25 : 0.3, w.z);
  float body = smoothstep(0.44, 0.39, w.x) * keep;
  float halo = smoothstep(0.56, 0.44, w.x) * keep;
  vec3 juice = mix(cream, fruit, 0.25);
  alb = mix(alb, juice, halo * 0.5);
  vec3 fc = fruit * (0.8 + 0.35 * vnoise(p * 700.0));
  if (slices > 0.5) fc = mix(fc, mix(fruit, vec3(1.0, 0.75, 0.75), 0.6), smoothstep(0.26, 0.08, w.x));
  alb = mix(alb, fc, body);
  rough = mix(rough, 0.2, body);
  h += body * 0.00015;
}
...
void shadeTop(vec3 p, float px, float edge, out vec3 alb, out float rough, out float h) {
  ...
  if (kind < 0.5) {
    ...
    rough = 0.55;
    h = ridges * patchy * 0.00007 + (vnoise(p * 900.0) - 0.5) * 0.00003;
  } else if (kind < 1.5) {
...
void shadeSide(vec3 p, float px, float y, out vec3 alb, out float rough, out float h) {
  ...
  if (kind < 0.5) {
    ...
    vec3 w = worley(vec3(ang * 38.0, y * 520.0, uSeed));
    float s = smoothstep(0.62, 0.35, w.x);
    vec3 choc = mix(vec3(0.2, 0.09, 0.045), vec3(0.34, 0.17, 0.08), w.z);
    alb = mix(c, choc * (0.85 + 0.3 * vnoise(p * 1200.0)), s);
    rough = mix(0.6, 0.45, s);
    h = s * (0.0007 + w.z * 0.0004);
  } else {
    // semi-naked: cream scraped thin, the sponge showing through between
    // the fillings
    ...
    float cover = smoothstep(0.38, 0.62, fbm(vec3(ang * 6.0, y * 120.0, uSeed)));
    cover = max(cover, 1.0 - isSponge);
    cover = max(cover, smoothstep(uTierH - 0.008, uTierH - 0.002, y));
    vec3 sponge = mix(uLayerA[idx], uLayerB[idx], 0.55 + 0.3 * vnoise(p * 600.0));
    vec3 w = worley(p * 650.0);
    sponge *= 1.0 - smoothstep(0.35, 0.1, w.x) * 0.35 * fadeAt(px, 650.0);
    alb = mix(sponge, c * (0.97 + 0.04 * vnoise(p * 300.0)), cover);
    rough = mix(0.85, 0.55, cover);
    h = (cover - 0.5) * 0.0005 - (1.0 - cover) * smoothstep(0.35, 0.1, w.x) * 0.0002;
  }
