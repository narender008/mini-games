// Loader: checks for WebGL2 and WebAssembly, downloads the engine, the
// physics and the game code with a progress bar (ES modules cannot report
// progress themselves), then starts.
...
async function boot() {
  ...
  if (!probe.getContext('webgl2') || typeof WebAssembly !== 'object') {
    fail('This game needs WebGL 2 and WebAssembly, which this browser or device does not provide. Try an up-to-date Chrome, Edge, Firefox or Safari.');
    return null;
  }
  progress(0.02, 'Opening the toy box');
  const expected = 5_300_000; // rough uncompressed size of the files above
  try {
